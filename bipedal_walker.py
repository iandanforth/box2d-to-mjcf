...
def extract_terrain_from_env(env):
    """
    This extracts a picklable list of objects from a
    BipedalWalker env instance.
    """
    py_objects = []
    for obj in env.drawlist:
        # Collision bodies
        for f in obj.fixtures:
            trans = f.body.transform
            py_obj = {
            ...
            if type(f.shape) is circleShape:
                print("Warning: Encountered a circleShape")
            else:
                path = [trans*v for v in f.shape.vertices]
                path = [v.tuple for v in path]
                print(path)
                py_obj["path"] = path
...
if __name__ == "__main__":
    ...
    env.reset()
    py_objects = extract_terrain_from_env(env)
    pickle.dump(py_objects, open("terrain-with-boxes-2.p", "wb"))
    steps = 0
    total_reward = 0
